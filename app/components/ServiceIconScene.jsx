...
import { Suspense, lazy, useLayoutEffect, useEffect, useMemo, useRef, useState } from "react";
...
// ──────────────────────────────────────────────
// Тяжёлая часть — выносим в lazy-компонент внутри файла
// ──────────────────────────────────────────────

const LazyHeavyScene = lazy(() => Promise.resolve({
  default: function HeavyScene({ progress }) {
    function clamp01(x) {
    ...
    useEffect(() => {
      ...
      invalidate(); // вызываем invalidate только при изменении progress
    }, [progress, invalidate]);
...
// ──────────────────────────────────────────────
// Основной компонент — лёгкий, ничего не грузит сразу
// ──────────────────────────────────────────────

export default function ServiceIconScene({ progress }) {
  const { ref, inView } = useInView({
    threshold: 0.01,
    triggerOnce: true,         // один раз загрузили — уже не выгружаем
    rootMargin: "300px 0px",   // начинаем грузить за ~300px до появления
  });

  return (
    <div ref={ref} className="w-full h-[clamp(280px,45vw,560px)]">
      {inView ? (
        <Suspense fallback={<div className="w-full h-full bg-[#0D0D0C] flex items-center justify-center text-white/40">Загрузка модели...</div>}>
          <Canvas
            camera={{ position: [0, 0, 4.0], fov: 50, near: 0.1, far: 100 }}
            frameloop="demand"          // ключевой момент — рендерим только по необходимости
            dpr={[1, 1.5]}
            gl={{ antialias: false }}
          >
            <color attach="background" args={["#0D0D0C"]} />
            <LazyHeavyScene progress={progress} />
          </Canvas>
        </Suspense>
      ) : (
